...
def open_wallet_menu(wallet):
    ...
    def refresh_balance():
        ...
        wallet.get_utxos()
        balance_label.config(text=f"Balance: {wallet.get_balance()}")
        utxo_list.delete(0, tk.END)
        for idx, utxo in enumerate(wallet.utxos):
            utxo_text = f"UTXO {idx + 1}: Amount: {utxo.amount}"
...
def open_blockchain_viewer():
    def update_block_details(index):
        """Update the block details displayed in the viewer."""
        if index < len(node.blockchain):
            ...
            block_index_label.config(text="Invalid block index.")
    
    def refresh_blockchain_view():
        """Refresh the blockchain viewer periodically."""
        # Adjust slider range if the blockchain size has changed
        block_slider.config(to=len(node.blockchain) - 1)

        # Keep displaying the current block's details
        index = block_slider.get()
        update_block_details(index)

        # Schedule the next refresh
        viewer.after(1000, refresh_blockchain_view)
    ...
    # Create the blockchain viewer window
    ...
    block_slider = tk.Scale(
        viewer, from_=0, to=max(0, len(node.blockchain) - 1), orient=tk.HORIZONTAL, length=400,
    ...
    # Initialize with the first block or show a message if blockchain is empty
    if node.blockchain:
        update_block_details(0)
    else:
        block_index_label.config(text="No blocks in the blockchain.")
    
    # Start refreshing the blockchain viewer
    refresh_blockchain_view()
